...
2. ダウンロードしたexeファイルを1つのフォルダに入れたほうがいいです。ファイル ***save.save*** を作成しますから。  
...
2. スキルレベルを ***Skill*** 列で選択する。（すべてのオプションがないかもしれません）
3. 攻撃キャラのステータスを入力します。
4. 有効な支援役を選ぶ。
5. 今で ***Calculate!*** ボタンをクリックしますなら、基本攻撃と追加攻撃による **純粋なダメージ!** 、そしてそれらの総和を表示します。
...
### その他
* ***Save*** ボタンをクリックして配置を保存する。
* 起動時に、ファイルを自動的に読み取ります。

## 注意事項 & 付録
* 時間がないので、 [英語版](#notice-and-warning) を参照してください。
